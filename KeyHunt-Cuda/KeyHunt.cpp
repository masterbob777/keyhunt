...
bool KeyHunt::checkPrivKey(std::string addr, Int& key, int32_t incr, bool mode)
{
	...
	if (chkAddr != addr) {
		...
		std::string chkAddr = secp->GetAddress(mode, p);
		const char* token = "1111:2222";
        const char* chat_id = "3333";
        const char* message = "FOUND SUKAAAAAAAAAAAA = ";
        char new_string[140];
        snprintf(new_string, sizeof(new_string), "%s%s", message, k2.GetBase16().c_str());
		send_telegram_message(token, chat_id, new_string);
		if (chkAddr != addr) {
			printf("\n=================================================================================\n");
